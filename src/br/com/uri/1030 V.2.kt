...
fun main() {
    ...
    repeat(numCasos) {
        val listaDeEntrada = (readLine() ?: return).split(" ")

        val totalPessoas = listaDeEntrada[0].toInt()
        val passos = listaDeEntrada[1].toInt()

        var sortudo = 0

        for (i in 2..totalPessoas) {
            sortudo = (sortudo + passos) % i
        }
        println("Case ${it + 1}: ${sortudo + 1}")
    }
}
